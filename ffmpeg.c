...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "q=%.1f ", q);
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "frame=%d fps=%.2f q=%.1f ",
    ...
    if (total_size < 0) snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
                                 ...
                                 "size=%.0fkB time=", total_size / 1024.0);
    if (pts < 0)
    ...
    if (bitrate < 0) {
        ...
        snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),"bitrate=%.0fkbits/s", bitrate);
    ...
    if (speed < 0) {
        ...
        snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf)," speed=%.2gx", speed);
